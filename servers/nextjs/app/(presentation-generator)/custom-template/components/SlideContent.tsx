  ...
  if (!module) {
    return (
      <div className="w-full aspect-[16/9] h-[720px] bg-red-50 text-red-700 p-4 rounded border border-red-200 text-sm whitespace-pre-wrap break-words flex flex-col items-center justify-center">
        <p className="text-center"> Falha ao renderizar o componente do slide. Verifique o console para mais detalhes.</p>
        <button onClick={() => retrySlide(slide.slide_number)} className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 flex items-center justify-center mt-6"> <RotateCcw className="w-4 h-4 mr-1" /> Reconstruir</button>
      </div>
    );
  }
