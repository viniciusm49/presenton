...
const SlideActionBar = ({
  ...
  const handleBlankSlide = () => {
    ...
    if (!templateId) {
      notify.error(
        "Não foi possível adicionar slide em branco",
        "Este slide não possui um modelo associado."
      );
      return;
    }
  ...
  const openTemplatePicker = () => {
    ...
    if (!templateId) {
      notify.error(
        "Não foi possível abrir os modelos",
        "Este slide não possui um modelo associado."
      );
      return;
    }
              ...
              >
                ...
                <div className="p-5">
                  <div className="max-h-[240px] min-h-[108px] overflow-auto whitespace-pre-wrap rounded-[12px] border border-[#EDEEEF] bg-[#FAFAFB] p-4 text-sm leading-relaxed text-[#333333]">
                    {speakerNote || "Nenhuma nota do orador para este slide."}
                  </div>
                </div>
              </PopoverContent>
            </Popover>}
