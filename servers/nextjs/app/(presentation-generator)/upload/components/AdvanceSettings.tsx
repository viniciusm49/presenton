...
const TONE_LABELS: Record<ToneType, string> = {
    [ToneType.Default]: "Padrão",
    [ToneType.Casual]: "Casual",
    [ToneType.Professional]: "Profissional",
    [ToneType.Funny]: "Divertido",
    [ToneType.Educational]: "Educacional",
    [ToneType.Sales_Pitch]: "Discurso de Vendas",
};

const VERBOSITY_LABELS: Record<VerbosityType, string> = {
    [VerbosityType.Concise]: "Conciso",
    [VerbosityType.Standard]: "Padrão",
    [VerbosityType.Text_Heavy]: "Detalhado",
};
...
const AdvanceSettings = ({
    ...
    return (
        <>
            <div className={cn(compact ? "flex justify-end" : "ml-auto")}>
                <ToolTip content="Configurações avançadas">
                    <button
                        ...
                        <div className="overflow-hidden rounded-[24px] border border-[#E7E9F2] bg-[#F3F3F6] shadow-[0_24px_80px_rgba(15,23,42,0.20)]">
                            <div className="flex items-start justify-between gap-4 bg-[#F8F8FA] px-6 py-[22px] min-[1800px]:px-8 min-[1800px]:py-7">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-[#808080] min-[1800px]:text-base">Ajuste o comportamento da apresentação</p>
                                </div>
                                ...
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="w-full">
                                        ...
                                        <Textarea
                                            ...
                                            placeholder="Oriente a IA: defina público-alvo, tom, pontos principais ou restrições."
                            ...
                            <div className="space-y-4 px-6 pb-5 pt-3.5 min-[1800px]:space-y-5 min-[1800px]:px-8 min-[1800px]:pb-7 min-[1800px]:pt-5">
                                <div className="flex items-center justify-between gap-3">
                                    <label className="font-syne text-sm font-semibold leading-none text-[#1F1D2A] min-[1800px]:text-base">Tom</label>
                                    <Select
                                    ...
                                    >
                                        <SelectTrigger className="w-[140px] rounded-xl border-[#DBDBE1] bg-white p-2.5 font-syne text-sm font-medium capitalize text-[#2C2B37] shadow-none focus:ring-0 focus-visible:ring-0 min-[1800px]:w-[160px] min-[1800px]:text-base">
                                            <SelectValue placeholder="Selecionar tom">
                                                {TONE_LABELS[advancedDraft.tone] || advancedDraft.tone}
                                            </SelectValue>
                                        </SelectTrigger>
                                ...
                                <div className="flex items-center justify-between gap-3">
                                    <label className="font-syne text-sm font-semibold leading-none text-[#1F1D2A] min-[1800px]:text-base">Detalhamento</label>
                                    <Select
                                    ...
                                    >
                                        <SelectTrigger className="w-[140px] rounded-xl border-[#DBDBE1] bg-white p-2.5 font-syne text-sm font-medium capitalize text-[#2C2B37] shadow-none focus:ring-0 focus-visible:ring-0 min-[1800px]:w-[160px] min-[1800px]:text-base">
                                            <SelectValue placeholder="Selecionar detalhamento">
                                                {VERBOSITY_LABELS[advancedDraft.verbosity] || advancedDraft.verbosity}
                                            </SelectValue>
                                        </SelectTrigger>
                                        <SelectContent className="z-[120] font-syne">
                                            {Object.values(VerbosityType).map((verbosity) => (
                                                <SelectItem key={verbosity} value={verbosity} className="text-sm font-medium min-[1800px]:text-base">
                                                    {VERBOSITY_LABELS[verbosity] || verbosity}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                ...
                                <div className="flex items-center justify-between gap-3">
                                    <label className="font-syne text-sm font-semibold leading-none text-[#1F1D2A] min-[1800px]:text-base">Slide de Título</label>
                                    <Switch
